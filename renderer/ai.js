...
function generateMockReadme(stats, aiStyle = 'balanced') {
  ...
  const sections = [
    {
      ...
      content: (stats.logos && stats.logos.length > 0 ? `<p align="center">\n  <img src="${stats.logos[0]}" alt="Logo" width="120" />\n</p>\n\n` : '') +
        `# ${stats.projectName}\n\n` +
  ...
  if (stats.screenshots && stats.screenshots.length > 0) {
    sections.push({
      id: 'screenshots',
      title: 'Screenshots',
      content: stats.screenshots.map(s => `![Screenshot](${s})`).join('\n\n')
    });
  }
...
async function generateAIExtractedReadme(stats, options) {
  ...
  try {
    ...
    window._lastThinkingLogs = response.thinkingText || null;
...
function parseAndExtractSections(resultText) {
  ...
  function sanitizeSections(result) {
    ...
    sanitized.forEach(sec => {
      ...
      if (sec.id !== 'screenshots' && sec.id !== 'title' && (contentIsBadges || (titleIsBadges && !sec.content.includes('\n')))) {
  ...
  throw new Error(`Could not parse AI response as a valid JSON array or object containing sections. Raw response (first 500 chars): ${cleaned.slice(0, 500)}`);
}
